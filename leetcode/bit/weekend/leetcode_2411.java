...
/*
    ...
    思路：
       ...
       2) 更加通用的模板；模板可以进行求解两道题
        ...
        用一个数组 ors 维护以 i 为左端点的子数组的按位或的结果，及其对应的子数组右端点的最小值。
        ors中的每个值
...
public class leetcode_2411 {
    ...
    public int[] smallestSubarrays(int[] nums) {
        int n = nums.length;
        int[] ans = new int[n];
        // 以i为左端点的子数组按位或的值 + 对应子数组右端点的最小值
        List<int[]> ors = new ArrayList<>();
        for (int i = n - 1; i >= 0; i--) {
            ors.add(new int[]{0, i});
            // k是代表第几个子数组？
            int k = 0;
            // 根据ors数组的值进行| 操作 根据前一个数的计算结果对当前的数进行 | 操作
            for (int[] or : ors) {
                // 将之前的结果与当前值进行或操作 这个存储的第一个数是
                // or[0] 是子数组右端的最小值
                or[0] |= nums[i];
                // 如果当前的值与第一个值相等，将合并下标，取最小的那个下标
                if (ors.get(k)[0] == or[0]) {
                    // 合并相同值，下标取最小的
                    ors.get(k)[1] = or[1];
                } else {
